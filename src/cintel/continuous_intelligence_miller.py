"""
continuous_intelligence_miller.py - Project script.
...
Paths (relative to repo root)
    ...
    OUTPUT FILE: artifacts/system_assessment_miller.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.continuous_intelligence_miller
...
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "system_assessment_miller.csv"
...
def main() -> None:
    ...
    summary_df = df.select(
        [
            pl.col("requests").mean().round(2).alias("avg_requests"),
            pl.col("errors").mean().round(2).alias("avg_errors"),
            pl.col("error_rate").mean().round(2).alias("avg_error_rate"),
            pl.col("avg_latency_ms").mean().round(2).alias("avg_latency_ms"),
        ]
    )
