"""
continuous_intelligence_transit.py - Project script.
...
- Data represents recent observations from a ride service.
- Each row represents one observation of activity.

- The CSV file includes these columns:
  - day: day of the observation
  - rides: number of rides requested
...
- Summarize the data's current state.
...
Paths (relative to repo root)
    ...
    OUTPUT FILE: artifacts/system_assessment_transit.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.continuous_intelligence_transit
...
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "transit_assessment.csv"
...
MAX_Z_SCORE: Final[float] = 2.0

# Choose window for rolling statistics
...
def main() -> None:
    ...
    df = df.with_columns(
        [
            pl.col("rides").rolling_mean(WINDOW).alias("roll_mean"),
            pl.col("rides").rolling_std(WINDOW).alias("roll_std"),
        ]
    )

    df = df.with_columns(
        [
            ((pl.col("rides") - pl.col("roll_mean")) / pl.col("roll_std")).alias(
                "rolling_z"
            )
        ]
    )
    ...
    anomalies_df = df.filter(pl.col("rolling_z").abs() > MAX_Z_SCORE)
    LOG.info(f"STEP 3. Using threshold: MAX_Z_SCORE={MAX_Z_SCORE}")
    ...
    summary_df = df.select(
        [
            pl.col("rides").mean().round(2).alias("avg_rides"),
            pl.col("rides").std().round(2).alias("rides_std"),
            pl.col("rolling_z").mean().round(2).alias("avg_rolling_z"),
        ]
    )

    # Add a simple assessment label
    summary_df = summary_df.with_columns(
        pl.when(pl.col("avg_rolling_z").abs() > MAX_Z_SCORE)
        .then(pl.lit("DEGRADED"))
        .otherwise(pl.lit("STABLE"))
        .alias("transit_state")
    )

    # Add a status_reason column to explain why the system is labeled as it is
    summary_df = summary_df.with_columns(
        pl.when(pl.col("avg_rolling_z").abs() > MAX_Z_SCORE)
        .then(pl.lit("Z-score too high"))
        .otherwise(pl.lit("Normal"))
        .alias("status_reason")
    )

    LOG.info("STEP 4. Transit assessment completed")

    # ----------------------------------------------------
    # STEP 5: SAVE TRANSIT ASSESSMENT
    # ----------------------------------------------------
    summary_df.write_csv(OUTPUT_FILE)

    LOG.info(f"STEP 5. Wrote transit assessment file: {OUTPUT_FILE}")
